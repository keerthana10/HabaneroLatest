...
public int index=0;
static double base1;
//public Map<String, double> linetaxmap=new Map<String, double>() {};  
//public Map<double, double> linetaxmap=new ; 
...
//Need to set RetailTicketLineInfo setLineTax with discountvalue - May18
        ...
        //System.out.println("RetailTicketInfo--Txn IDS"+line.getLineId());
        
        System.out.println("RetailTicketInfo-changes for all txn.-Txn IDS"+ticket.getTransactionID());
           System.out.println("RetailTicketInfo--ID"+ticket.getId());
            System.out.println("RetailTicketInfo--OrderID"+ticket.getOrderId());
              System.out.println("RetailTicketInfoPlaceID"+ticket.getPlaceId());
                  System.out.println("RetailTicketInfo SOSORDER ID"+ticket.getSosOrderId());
                       System.out.println("RetailTicketInfo ParentID"+ticket.getParentId());
        TaxesLogicElement taxesapplied = getTaxesApplied(line.getTaxInfo());
       // System.out.println("RetailTicketInfo--Txn IDS"+line.getTicketId());
        ...
        if (ticket.iscategoryDiscount()) {
           // return calculateLineTaxes(line.getSubValueOnProductCat(), taxesapplied, ticket);
            return calculateLineTaxes(line.getSubValueOnProductCat(), taxesapplied, ticket,line);
        } else {
            //if its bill level discount
           // return calculateLineTaxes(line.getSubValue(), taxesapplied, ticket);
            return calculateLineTaxes(line.getSubValue(), taxesapplied, ticket,line);
        }
    }
    ...
    private List<TicketTaxInfo> calculateLineTaxes(double base, TaxesLogicElement taxesapplied, RetailTicketInfo ticket,RetailTicketLineInfo line) {
    ...
    public void calculatelinetaxamount(double base,double returntax)
    {   
    Map<Double,Double> linetaxmap=new HashMap<Double,Double>();  
        ...
        double tempfetchtaxline=getFetchtaxline();
             if(base==tempfetchtaxline){
                 ...
                 linetaxmap.put(base,returntax);
                  sumlinetax+=returntax;
              
             
              // System.out.println("index---"+index+"--"+sumlinetax);
             }
 ...
 private List<TicketTaxInfo> calculateLineTaxes(double base, TaxesLogicElement taxesapplied,RetailTicketLineInfo line) {
        List<TicketTaxInfo> linetaxes = new ArrayList<TicketTaxInfo>();
        //true always?
        ...
        if (taxesapplied.getSons().isEmpty()) {
            ...
            //calculating child tax value 
             ...
             System.out.println("Ticket Line -getID--"+ticket.getId());
               //System.out.println("Ticket Line -ItcketID--"+ticket.getTicketId());
           System.out.println("Ticket Line -Txn . ID"+ticket.getTransactionID());
           System.out.println("Ticket Line -Txn . ID"+line.getTbl_orderId());
            // System.out.println("Ticket Line -Txn . ID"+ticket.);
          System.out.println("---RETURN TAX--"+returntax);
          //May 18 -next 2 lines
           // setFetchtaxline(base);            
             //calculatelinetaxamount(base,returntax);
